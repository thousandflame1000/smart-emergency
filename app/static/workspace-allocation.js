...
function renderAllocationResult(){
  ...
  if(dbCount)html=`<section class="comparison-section"><h3>送到調度</h3><p class="muted">其中 ${dbCount} 筆來自平台資料庫。送出後會在後台「調度」出現為「待核准」建議、並保留該份物資；不會通知志工，須管理員確認才會派遣。</p>${isAdmin()?`<button id="send-allocation" class="primary"><i data-lucide="send"></i>送到調度（待核准）</button><div id="send-allocation-result" role="status"></div>`:'<p class="muted">送到調度僅限管理員。</p>'}</section>`+html;
